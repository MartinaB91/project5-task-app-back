...
class ProfileModelTest(TestCase):
    ...
    def test_profile_user_created_at(self):
        ...
        """
        profile = Profile.objects.first()
        created_at_datetime = datetime.datetime(
            profile.created_at.year, profile.created_at.month, profile.created_at.day
        )
        assert created_at_datetime.strftime("%Y-%m-%d") == date.today().strftime(
            "%Y-%m-%d"
        )
    ...
    def test_profile_user_updated_at(self):
        ...
        """
        profile = Profile.objects.first()
        updated_at_datetime = datetime.datetime(
            profile.updated_at.year, profile.updated_at.month, profile.updated_at.day
        )
        assert updated_at_datetime.strftime("%Y-%m-%d") == date.today().strftime(
            "%Y-%m-%d"
        )
