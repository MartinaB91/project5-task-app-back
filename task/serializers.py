...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # If payload has no title (only containing info about status)
        # and status id Done.
        if (
            ...
            if task.assigned is None:
                ...
                # If user remove task done. Assigned family member
                # will loose star points and 1 closed task
                if task.status == "Done":
                    task.status = "Todo"
                    assigned_family_member.closed_tasks = (
                        assigned_family_member.closed_tasks - 1
                    )
                    # A family member can't have negative scores on
                    # their scoreboard. If negative, set to 0
                ...
                # Assigned family member will gain star points and 1
                # point will be added to closed task.
        ...
        # If payload has no title ans status
        # (only containing info about assignment).
        elif (
    ...
    # Override create to be able to change familymember
    # score if is assigned at create form
