...
class FamilyMemberSerializer(serializers.ModelSerializer):
    belongs_to_profile = serializers.StringRelatedField(source='user.profile.id')
    name = serializers.CharField(max_length=200)
    #todo: Check why StringRelatedField gives KeyError when CharField work??
    # family_member_img = serializers.StringRelatedField(source='user.profile.family_member_img.url')
